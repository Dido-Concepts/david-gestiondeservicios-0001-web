...
function ContentLink (param: { location: Omit<LocationModel, 'openingHours'> }) {
  return (
        <Card className={`
            flex flex-col md:flex-row items-start md:items-center p-4 space-y-4 md:space-y-0 md:space-x-4  ${param.location.status ? 'bg-gray-100 opacity-50' : 'cursor-pointer transition-shadow hover:shadow-lg'
            }`}>
            ...
            <CardContent className="w-full md:w-2/3">
                <CardHeader className="p-0">
                    <CardTitle className="text-lg font-bold">{param.location.name}</CardTitle>
                </CardHeader>
                    ...
                    <DropdownMenuContent>
                        {
                            !param.location.status
                              ? (
                                    <Link href={`/dashboard/location-management/${param.location.id}/edit`}>
                        ...
                        <DropdownMenuItem>{
                            param.location.status ? 'Activar' : 'Desactivar'
                        }</DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenu>
            </CardFooter>
...
export function ListLocations (param: { pageIndex: number, pageSize: number }) {
  const { data } = useSuspenseQuery({
    queryKey: [QUERY_KEYS_LOCATION_MANAGEMENT.LMListLocations, param.pageIndex, param.pageSize],
    queryFn: () => getLocations({ pageIndex: param.pageIndex, pageSize: param.pageSize })
  })
